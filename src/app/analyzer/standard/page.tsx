...
import { Upload, X, Loader2, CheckCircle, AlertTriangle, Image as ImageIcon, Activity, Droplets, Eye, Sparkles, Zap, DollarSign, RefreshCw, Clock, Heart, TrendingUp, Shield, Info, AlertCircle, Stethoscope, Pill, Calendar, BookOpen, Smartphone, Camera } from "lucide-react";
...
export default function StandardAnalyzerPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (isRateLimit) {
          ...
          toast.error(`⏱️ Rate Limit Reached`, {
            ...
            duration: 8000,
            action: suggestedProviders.length > 0 ? {
              label: `Switch to ${suggestedProviders[0]}`,
              onClick: () => {
                const providerKey = suggestedProviders[0].toLowerCase().includes('gemini') ? 'gemini' : 'groq';
                setAiProvider(providerKey as AIProvider);
                setRateLimitInfo(null);
              }
            } : undefined
          });
          ...
          toast.dismiss(loadingToast);
          toast.error(`${providerName} Error`, {
      ...
      const errorMsg = `Failed to connect to ${providerName}. Check console for details.`;
      setError(errorMsg);
      toast.dismiss(loadingToast);
  ...
  const getSeverityBg = (severity: string) => {
    switch (severity) {
      case "none": return "bg-green-100 dark:bg-green-900/30 border-green-200 dark:border-green-800";
  ...
  const getHealthCondition = (health: number): { label: string; color: string; bgColor: string; icon: any } => {
    if (health >= 90) {
      return {
        label: "Super Healthy",
  ...
  const isSuperHealthy = (result: AnalysisResult) => {
    return result.stage === 0 || (result.healthPercentage >= 90 && result.severity === "none");
  };

  const isCriticalCondition = (result: AnalysisResult) => {
    return result.stage === 3 || result.healthPercentage < 45 || result.severity === "high";
  };
            ...
            <AlertDescription className="space-y-3">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-amber-700 dark:text-amber-300 mt-2">
                  💡 Tip: Each provider has independent rate limits. You can freely switch between them!
                </p>
              </div>
            </AlertDescription>
          </Alert>
              ...
              <div className="flex flex-col items-center justify-center space-y-4 py-12">
                ...
                <div className="text-center space-y-2">
                  ...
                  <p className="text-xs text-green-600 dark:text-green-400 font-medium">
                    📸 Best results: Clear leaf photos with good lighting
                  </p>
                </div>
                <input
                  ref={fileInputRef}
                ...
                {result.noLeafDetected ? (
                  ...
                  <>
                    ...
                    <Card className="border-2 border-green-200 dark:border-green-800 shadow-xl">
                      <CardHeader>
                        <CardTitle className="text-2xl">{result.primaryDisease}</CardTitle>
                        <CardDescription className="text-base">{result.category}</CardDescription>
                      </CardHeader>
                      <CardContent className="space-y-6">
                        ...
                        <div className="space-y-3">
                          <div className="flex justify-between items-center">
                            ...
                            <Badge className={getSeverityBg(result.severity)}>
                          ...
                          {result.possibleDiseases.map((disease: any, index: number) => (
                            <div key={index} className="p-4 bg-white dark:bg-gray-900 rounded-lg border-2 border-orange-200 dark:border-orange-800 shadow-sm">
                              <div className="flex items-start justify-between gap-4 mb-2">
                                <h5 className="font-bold text-lg text-orange-700 dark:text-orange-300">
                                  {typeof disease === 'string' ? disease : disease.name}
                                </h5>
                                {typeof disease === 'object' && disease.likelihood && (
                                  <Badge className="bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-300 text-sm px-3 py-1">
                        ...
                        <CardContent className="space-y-4">
                          {result.causes.map((causeInfo, index) => (
                            <div key={index} className="p-4 bg-white dark:bg-gray-900 rounded-lg border-2 border-purple-200 dark:border-purple-800 shadow-sm">
                              <div className="space-y-3">
                                <div>
                                  <h5 className="font-bold text-base text-purple-700 dark:text-purple-300 mb-1">
                                ...
                                <div className="pl-4 border-l-4 border-purple-300 dark:border-purple-700">
                                  <p className="text-sm text-muted-foreground leading-relaxed">
                                    <strong className="text-purple-600 dark:text-purple-400">Why:</strong> {causeInfo.explanation}
                                  </p>
                                </div>
                              </div>
                            </div>
