...
export async function analyzeImageGemini(base64Image: string) {
  ...
  const base64Only = base64Image.includes(',') 
    ? base64Image.split(',')[1] 
    : base64Image;
  ...
  if (analysis.healthPercentage >= 90) {
    ...
    if (!analysis.category?.includes("Excellent")) {
      analysis.category = "Perfect Health";
    }
  } else if (analysis.healthPercentage < 45 || analysis.stage === 3) {
    analysis.stage = 3;
    analysis.severity = "high";
    if (!analysis.category?.includes("Critical")) {
      analysis.category = "Critical - Emergency Intervention";
    }
  }
